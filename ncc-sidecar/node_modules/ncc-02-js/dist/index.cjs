...
var NCC02Builder = class {
  ...
  createAttestation(options) {
    ...
    const expiry = now2 + validDays * 24 * 60 * 60;
    const event = {
...
var SimplePool = class extends AbstractSimplePool {
  constructor(options) {
    super({ verifyEvent: verifyEvent2, websocketImplementation: _WebSocket2, ...options });
  }
};
var nip19_exports = {};
...
function fastEventHash(evt) {
  return bytesToHex2(
    sha2562(utf8Encoder3.encode(JSON.stringify([0, evt.pubkey, evt.created_at, evt.kind, evt.tags, evt.content])))
  );
}
var nip17_exports = {};
...
function hashPayload(payload) {
  const hash3 = sha2562(utf8Encoder3.encode(JSON.stringify(payload)));
    ...
    return this.events.filter((event) => {
      ...
      for (const key in filter) {
        if (key.startsWith("#")) {
          ...
          if (!filterValues.some((fv) => eventTagValues.includes(fv))) {
            return false;
          }
        }
      }
